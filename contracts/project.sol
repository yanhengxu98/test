...
contract Project {
    ...
    function createPayment(string _description, uint _amount, address _receiver) ownerOnly {
    ...
    function doPayment(uint index) ownerOnly {

        Payment storage payment = payments[index];

        require(!payment.completed);
        require(this.balance >= payment.amount);
        require(payment.voterCount > (investors.length / 2));
